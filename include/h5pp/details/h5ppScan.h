...
namespace h5pp::scan {
    ...
    inline void inferAttrInfo(AttrInfo &info, const h5x &loc, const Options &options, const PropertyLists &plists = PropertyLists()) {
        ...
        if(info.attrExists.value()) readAttrInfo(info, loc, options, plists); // Table exists so we can read properties from file
        else makeAttrInfo(info, loc, options, plists);
    }
    ...
    [[nodiscard]] inline TableFieldInfo getTableFieldInfo(const h5x              &loc,
        ...
        TableFieldInfo info;
        hid::h5f       h5File;
        if constexpr(std::is_same_v<h5x, hid::h5f>) h5File = loc;
        else h5File = H5Iget_file_id(loc);
    ...
    inline void makeTableInfo(h5pp::TableInfo     &info,
        ...
        if(not info.fieldClasses){
            info.fieldClasses   = std::vector<H5T_class_t>(n_fields);
            for(unsigned int idx = 0; idx < n_fields; idx++)
                info.fieldClasses.value()[idx] = H5Tget_member_class(info.h5Type.value(), idx);
        }
    ...
    inline void inferTableInfo(TableInfo &info, const h5x &loc, const Options &options, const PropertyLists &plists = PropertyLists()) {
        ...
        if(info.tableExists.value()) readTableInfo(info, loc, options, plists); // Table exists so we can read properties from file
        else if(not info.tableExists.value() and info.tableTitle) makeTableInfo(info, loc, options, info.tableTitle.value(), plists);
