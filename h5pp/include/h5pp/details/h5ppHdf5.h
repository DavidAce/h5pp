...
namespace h5pp::Hdf5 {
    ...
    inline bool checkIfLinkExistsRecursively(hid_t file, const std::string &path) {
        std::stringstream        path_stream(path);
        std::vector<std::string> split_path;
        std::string              part;
        while(std::getline(path_stream, part, '/')) {
            if(not part.empty()) { split_path.push_back(part); }
        }
        bool        exists = true;
    ...
    inline hid_t openLink(hid_t file, const std::string &linkName) {
        ...
        } catch(std::exception &ex) { throw std::runtime_error("openLink: " + std::string(ex.what())); }
    }
    ...
    inline void setExtentDataset(hid_t file, const DatasetProperties &props) {
        try {
            if(not props.extendable) { h5pp::Logger::log->critical("Called setExtentDataset for non-extendable dataset"); }
    ...
    template<typename DataType> void extendDataset(hid_t file, const DataType &data, const std::string &datasetRelativeName) {
    ...
    inline void createDatasetLink(hid_t file, hid_t plist_lncr, const DatasetProperties &props) {
        if(not h5pp::Hdf5::checkIfLinkExistsRecursively(file, props.dsetName)) {
            ...
            hid_t dataset = H5Dcreate(file, props.dsetName.c_str(), props.dataType, props.dataSpace, plist_lncr, dset_cpl, H5P_DEFAULT);
            H5Pclose(dset_cpl);
            closeLink(dataset);
        }
    }
    ...
    inline std::vector<std::string> getContentsOfGroup(hid_t file, const std::string &groupName) {
