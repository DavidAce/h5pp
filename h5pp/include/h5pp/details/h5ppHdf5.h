...
namespace h5pp::Hdf5 {
    ...
    inline std::vector<std::string_view>
        getAttributeNames(Hid::h5f &file, std::string_view linkPath, std::optional<bool> linkExists = std::nullopt, const PropertyLists &plists = PropertyLists()) {
        Hid::h5o                      link      = openLink(file, linkPath, linkExists, plists);
        unsigned int                  num_attrs = H5Aget_num_attrs(link);
        std::vector<std::string_view> attrNames;
    ...
    inline bool checkIfAttributeExists(Hid::h5f &           file,
                                       std::string_view     linkPath,
                                       std::string_view     attributename,
                                       std::optional<bool>  linkExists = std::nullopt,
                                       const PropertyLists &plists     = PropertyLists()) {
        Hid::h5o link = openLink(file, linkPath, linkExists, plists);
        h5pp::Logger::log->trace("Checking if attribute [{}] exitst in link [{}]", attributename, linkPath);
        bool         exists    = false;
        unsigned int num_attrs = H5Aget_num_attrs(link);
    ...
    inline std::vector<hsize_t> getMaxDims(const DatasetProperties &dsetProps) {
        ...
        H5Sget_simple_extent_dims(dsetProps.dataSpace.value(), nullptr, maxDims.data());
        return maxDims;
    }
    ...
    inline void writeSymbolicLink(Hid::h5f &file, std::string_view src_path, std::string_view tgt_path, const PropertyLists &plists = PropertyLists()) {
        if(checkIfLinkExists(file, src_path)) {
    ...
    inline void setDataSpaceExtent(const DatasetProperties &dsetProps) {
        ...
        if(dsetProps.layout.value() == H5D_CHUNKED) {
            ...
            herr_t err = H5Sset_extent_simple(dsetProps.dataSpace.value(), dsetProps.ndims.value(), dsetProps.dims.value().data(), maxDims.data());
            if(err < 0) {
                H5Eprint(H5E_DEFAULT, stderr);
    ...
    void writeDataset(const Hid::h5f &file, const DataType &data, const DatasetProperties &props, const PropertyLists &plists = PropertyLists()) {
        h5pp::Logger::log->debug("Writing dataset: [{}] | size {} | bytes {} | ndims {} | dims {}",
