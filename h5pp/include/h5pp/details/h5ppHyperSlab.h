    ...
    class HyperSlab {
        ...
        explicit HyperSlab(const hid::h5s &space) {
            ...
            if(select_type.value() == H5S_SEL_HYPERSLABS) {
                offset = std::vector<hsize_t>((size_t) rank, 0);
                extent = std::vector<hsize_t>((size_t) rank, 0);
                stride = std::vector<hsize_t>((size_t) rank, 0);
                block  = std::vector<hsize_t>((size_t) rank, 0);
