...
//        if (H5Tequal(get_DataType<DataType>(), H5T_COMPLEX_DOUBLE) and not std::is_same<std::vector<H5T_COMPLEX_STRUCT>, DataType>::value) {
        if (h5pp::Type::Check::isComplex<DataType>() and not  h5pp::Type::Check::isVectorOf_H5T_COMPLEX_STRUCT<DataType>()) {
            //If complex, convert data to complex struct H5T_COMPLEX_DOUBLE
//            auto new_temp = convert_complex_data(temp);
            auto new_temp = h5pp::Utils::convert_complex_data(temp);
        ...
        }else if (h5pp::Type::Check::isComplex<DataType>() and not  h5pp::Type::Check::isVectorOf_H5T_COMPLEX_STRUCT<DataType>()) {
            //If complex, convert data to complex struct H5T_COMPLEX_DOUBLE
//            auto new_data = convert_complex_data(data);
//            auto temp     = h5pp::Utils::complex_vector_to_rowmajor(data);
            auto new_temp = h5pp::Utils::convert_complex_data(data);
            write_dataset(new_temp, props);
        }else{
            write_dataset(data, props);
        }
