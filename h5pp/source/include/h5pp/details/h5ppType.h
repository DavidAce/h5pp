...
namespace h5pp{
    namespace Type{
        ...
        constexpr hid_t get_DataType() {
            ...
            if constexpr (std::is_same<DataType, Complex::H5T_COMPLEX_STRUCT<int>>::value)              {return  H5Tcopy(Complex::H5T_COMPLEX_INT);}
            if constexpr (std::is_same<DataType, Complex::H5T_COMPLEX_STRUCT<long>>::value)             {return  H5Tcopy(Complex::H5T_COMPLEX_LONG);}
            if constexpr (std::is_same<DataType, Complex::H5T_COMPLEX_STRUCT<unsigned int>>::value)     {return  H5Tcopy(Complex::H5T_COMPLEX_UINT);}
