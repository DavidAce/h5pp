...
function(install_package pkg_name)
    ...
    if(${pkg_name}_FOUND)
        ...
        foreach(tgt ${PKG_TARGET_HINTS})
            if(TARGET ${tgt})
                set(${pkg_name}_TARGET ${tgt})
                set(${pkg_name}_TARGET ${tgt} PARENT_SCOPE)
                set(${pkg_name}_FOUND TRUE PARENT_SCOPE)
                set(${pkg_find_name}_TARGET ${tgt} PARENT_SCOPE)
                set(${pkg_find_name}_FOUND TRUE PARENT_SCOPE)
                mark_as_advanced(${pkg_name}_TARGET)
                break()
            endif()
        endforeach()
        if(${pkg_name}_TARGET)
            message(DEBUG "Found ${pkg_name}: [${${pkg_name}_TARGET}]")
        else()
